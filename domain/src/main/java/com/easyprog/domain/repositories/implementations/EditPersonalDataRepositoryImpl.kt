...
import com.easyprog.data.remote.providers.EditPersonalDataProviderImpl
...
class EditPersonalDataRepositoryImpl(val roomDatabase: RoomDatabaseApp): EditPersonalDataRepository {
    ...
    override suspend fun editPersonalDataAsync(token: String, name: String, surname: String, email: String, login: String): Deferred<EditPersonalDataApi> {
        val editPersonalDataProviderImpl = EditPersonalDataProviderImpl()

        return try {
            val editPersonalDataProvider = editPersonalDataProviderImpl
